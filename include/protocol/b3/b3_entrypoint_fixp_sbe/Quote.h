...
#ifndef _B3_ENTRYPOINT_FIXP_SBE_QUOTE_CXX_H_
#define _B3_ENTRYPOINT_FIXP_SBE_QUOTE_CXX_H_
...
public:
    static const std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(97);
    ...
    Quote(char *buffer, const std::uint64_t bufferLength) :
        Quote(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    Quote(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        Quote(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    Quote &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        return *this = Quote(buffer, offset, bufferLength, sbeBlockLength(), sbeSchemaVersion());
    }

    Quote &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    Quote &wrapForDecode(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = Quote(buffer, offset, bufferLength, actingBlockLength, actingVersion);
    }
    ...
    {
        Quote skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    {
        return MessageType::Value::Quote;
    }
    ...
    {
        return static_cast<std::uint8_t>(MessageType::Value::Quote);
    }
...
public:
    ...
    Quote &securityID(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    Quote &quoteID(const std::uint64_t value) SBE_NOEXCEPT
    {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
...
public:
    ...
    {
        m_price.wrap(m_buffer, m_offset + 44, m_actingVersion, m_bufferLength);
        return m_price;
    }
    ...
    Quote &orderQty(const std::uint64_t value) SBE_NOEXCEPT
    {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    Quote &side(const Side::Value value) SBE_NOEXCEPT
    {
        char val = (value);
    ...
    Quote &settlType(const SettlType::Value value) SBE_NOEXCEPT
    {
        char val = (value);
    ...
    {
        std::uint32_t val;
        std::memcpy(&val, m_buffer + m_offset + 62, sizeof(std::uint32_t));
        return SBE_LITTLE_ENDIAN_ENCODE_32(val);
    }

    Quote &account(const std::uint32_t value) SBE_NOEXCEPT
    {
        std::uint32_t val = SBE_LITTLE_ENDIAN_ENCODE_32(value);
        std::memcpy(m_buffer + m_offset + 62, &val, sizeof(std::uint32_t));
        return *this;
    }
    ...
    SBE_NODISCARD char senderLocation(const std::uint64_t index) const
    {
        ...
        char val;
        std::memcpy(&val, m_buffer + m_offset + 66 + (index * 1), sizeof(char));
        return (val);
    }

    Quote &senderLocation(const std::uint64_t index, const char value)
    {
        if (index >= 10)
        {
        ...
        char val = (value);
        std::memcpy(m_buffer + m_offset + 66 + (index * 1), &val, sizeof(char));
        return *this;
    }
    ...
    std::uint64_t getSenderLocation(char *const dst, const std::uint64_t length) const
    {
        ...
        std::memcpy(dst, m_buffer + m_offset + 66, sizeof(char) * static_cast<std::size_t>(length));
        return length;
    }

    Quote &putSenderLocation(const char *const src) SBE_NOEXCEPT
    {
        std::memcpy(m_buffer + m_offset + 66, src, sizeof(char) * 10);
        return *this;
    }

    SBE_NODISCARD std::string getSenderLocationAsString() const
    {
        const char *buffer = m_buffer + m_offset + 66;
        std::size_t length = 0;
    ...
    #if __cplusplus >= 201703L
    Quote &putSenderLocation(const std::string_view str)
    {
        ...
        std::memcpy(m_buffer + m_offset + 66, str.data(), srcLength);
        for (std::size_t start = srcLength; start < 10; ++start)
        {
            m_buffer[m_offset + 66 + start] = 0;
        }

        return *this;
    }
    #else
    Quote &putSenderLocation(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 10)
        {
        ...
        std::memcpy(m_buffer + m_offset + 66, str.c_str(), srcLength);
        for (std::size_t start = srcLength; start < 10; ++start)
        {
            m_buffer[m_offset + 66 + start] = 0;
        }
    ...
    Quote &enteringTrader(const std::uint64_t index, const char value)
    {
        if (index >= 5)
        {
    ...
    Quote &putEnteringTrader(const char *const src) SBE_NOEXCEPT
    {
        std::memcpy(m_buffer + m_offset + 76, src, sizeof(char) * 5);
        return *this;
    }
    ...
    #if __cplusplus >= 201703L
    Quote &putEnteringTrader(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 5)
        {
    ...
    #else
    Quote &putEnteringTrader(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 5)
        {
    ...
    SBE_NODISCARD char executingTrader(const std::uint64_t index) const
    {
        ...
        char val;
        std::memcpy(&val, m_buffer + m_offset + 81 + (index * 1), sizeof(char));
        return (val);
    }

    Quote &executingTrader(const std::uint64_t index, const char value)
    {
        if (index >= 5)
        {
        ...
        char val = (value);
        std::memcpy(m_buffer + m_offset + 81 + (index * 1), &val, sizeof(char));
        return *this;
    }
    ...
    std::uint64_t getExecutingTrader(char *const dst, const std::uint64_t length) const
    {
        ...
        std::memcpy(dst, m_buffer + m_offset + 81, sizeof(char) * static_cast<std::size_t>(length));
        return length;
    }

    Quote &putExecutingTrader(const char *const src) SBE_NOEXCEPT
    {
        std::memcpy(m_buffer + m_offset + 81, src, sizeof(char) * 5);
        return *this;
    }

    SBE_NODISCARD std::string getExecutingTraderAsString() const
    {
        const char *buffer = m_buffer + m_offset + 81;
        std::size_t length = 0;
    ...
    #if __cplusplus >= 201703L
    Quote &putExecutingTrader(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 5)
        {
        ...
        std::memcpy(m_buffer + m_offset + 81, str.data(), srcLength);
        for (std::size_t start = srcLength; start < 5; ++start)
        {
            m_buffer[m_offset + 81 + start] = 0;
        }

        return *this;
    }
    #else
    Quote &putExecutingTrader(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 5)
        {
        ...
        std::memcpy(m_buffer + m_offset + 81, str.c_str(), srcLength);
        for (std::size_t start = srcLength; start < 5; ++start)
        {
            m_buffer[m_offset + 81 + start] = 0;
        }
...
public:
    ...
    {
        m_fixedRate.wrap(m_buffer, m_offset + 86, m_actingVersion, m_bufferLength);
        return m_fixedRate;
    }
    ...
    {
        char val;
        std::memcpy(&val, m_buffer + m_offset + 94, sizeof(char));
        return (val);
    }
    ...
    Quote &executeUnderlyingTrade(const ExecuteUnderlyingTrade::Value value) SBE_NOEXCEPT
    {
        char val = (value);
        std::memcpy(m_buffer + m_offset + 94, &val, sizeof(char));
        return *this;
    }
    ...
    {
        std::uint16_t val;
        std::memcpy(&val, m_buffer + m_offset + 95, sizeof(std::uint16_t));
        return SBE_LITTLE_ENDIAN_ENCODE_16(val);
    }

    Quote &daysToSettlement(const std::uint16_t value) SBE_NOEXCEPT
    {
        std::uint16_t val = SBE_LITTLE_ENDIAN_ENCODE_16(value);
        std::memcpy(m_buffer + m_offset + 95, &val, sizeof(std::uint16_t));
        return *this;
    }
    ...
    Quote &putQuoteReqID(const char *src, const std::uint8_t length)
    {
        std::uint64_t lengthOfLengthField = 1;
        std::uint64_t lengthPosition = sbePosition();
    ...
    Quote &putQuoteReqID(const std::string &str)
    {
        if (str.length() > 20)
        {
    ...
    #if __cplusplus >= 201703L
    Quote &putQuoteReqID(const std::string_view str)
    {
        if (str.length() > 20)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    Quote &putDeskID(const char *src, const std::uint8_t length)
    {
        std::uint64_t lengthOfLengthField = 1;
        std::uint64_t lengthPosition = sbePosition();
        std::uint8_t lengthFieldValue = (length);
    ...
    Quote &putDeskID(const std::string &str)
    {
        if (str.length() > 20)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    #if __cplusplus >= 201703L
    Quote &putDeskID(const std::string_view str)
    {
        if (str.length() > 20)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    Quote &putMemo(const char *src, const std::uint8_t length)
    {
        std::uint64_t lengthOfLengthField = 1;
        std::uint64_t lengthPosition = sbePosition();
        std::uint8_t lengthFieldValue = (length);
    ...
    Quote &putMemo(const std::string &str)
    {
        if (str.length() > 40)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    #if __cplusplus >= 201703L
    Quote &putMemo(const std::string_view str)
    {
        if (str.length() > 40)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, Quote &_writer)
{
    Quote writer(
        _writer.m_buffer,
        _writer.m_offset,
