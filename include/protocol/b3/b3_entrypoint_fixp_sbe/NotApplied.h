...
#ifndef _B3_ENTRYPOINT_FIXP_SBE_NOTAPPLIED_CXX_H_
#define _B3_ENTRYPOINT_FIXP_SBE_NOTAPPLIED_CXX_H_
...
public:
    ...
    static const std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(8);
    ...
    NotApplied(char *buffer, const std::uint64_t bufferLength) :
        NotApplied(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    NotApplied(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        NotApplied(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    NotApplied &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        return *this = NotApplied(buffer, offset, bufferLength, sbeBlockLength(), sbeSchemaVersion());
    }

    NotApplied &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    NotApplied &wrapForDecode(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = NotApplied(buffer, offset, bufferLength, actingBlockLength, actingVersion);
    }

    NotApplied &sbeRewind()
    {
    ...
    {
        NotApplied skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    {
        return MessageType::Value::NotApplied;
    }
    ...
    {
        return static_cast<std::uint8_t>(MessageType::Value::NotApplied);
    }
    ...
    NotApplied &fromSeqNo(const std::uint32_t value) SBE_NOEXCEPT
    {
    ...
    NotApplied &count(const std::uint32_t value) SBE_NOEXCEPT
    {
        std::uint32_t val = SBE_LITTLE_ENDIAN_ENCODE_32(value);
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, NotApplied &_writer)
{
    NotApplied writer(
        _writer.m_buffer,
        _writer.m_offset,
