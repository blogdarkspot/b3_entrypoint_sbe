...
#ifndef _B3_ENTRYPOINT_FIXP_SBE_ESTABLISHREJECT_CXX_H_
#define _B3_ENTRYPOINT_FIXP_SBE_ESTABLISHREJECT_CXX_H_
...
public:
    static const std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(26);
    ...
    EstablishReject(char *buffer, const std::uint64_t bufferLength) :
        EstablishReject(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    EstablishReject(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        EstablishReject(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    EstablishReject &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        return *this = EstablishReject(buffer, offset, bufferLength, sbeBlockLength(), sbeSchemaVersion());
    }

    EstablishReject &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    EstablishReject &wrapForDecode(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = EstablishReject(buffer, offset, bufferLength, actingBlockLength, actingVersion);
    }

    EstablishReject &sbeRewind()
    {
    ...
    {
        EstablishReject skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    {
        return MessageType::Value::EstablishReject;
    }
    ...
    {
        return static_cast<std::uint8_t>(MessageType::Value::EstablishReject);
    }
    ...
    EstablishReject &sessionID(const std::uint32_t value) SBE_NOEXCEPT
    {
    ...
    EstablishReject &sessionVerID(const std::uint64_t value) SBE_NOEXCEPT
    {
...
public:
    ...
    SBE_NODISCARD static const char *establishmentRejectCodeMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "required";
    ...
    static SBE_CONSTEXPR std::uint16_t establishmentRejectCodeId() SBE_NOEXCEPT
    {
        return 35532;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t establishmentRejectCodeSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool establishmentRejectCodeInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t establishmentRejectCodeEncodingOffset() SBE_NOEXCEPT
    {
        return 20;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t establishmentRejectCodeEncodingLength() SBE_NOEXCEPT
    {
        return 1;
    }

    SBE_NODISCARD std::uint8_t establishmentRejectCodeRaw() const SBE_NOEXCEPT
    {
        std::uint8_t val;
    ...
    SBE_NODISCARD EstablishRejectCode::Value establishmentRejectCode() const
    {
        std::uint8_t val;
        std::memcpy(&val, m_buffer + m_offset + 20, sizeof(std::uint8_t));
        return EstablishRejectCode::get((val));
    }

    EstablishReject &establishmentRejectCode(const EstablishRejectCode::Value value) SBE_NOEXCEPT
    {
    ...
    EstablishReject &lastIncomingSeqNo(const std::uint32_t value) SBE_NOEXCEPT
    {
        std::uint32_t val = SBE_LITTLE_ENDIAN_ENCODE_32(value);
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, EstablishReject &_writer)
{
    EstablishReject writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "EstablishReject", )";
    ...
    builder << ", ";
    builder << R"("establishmentRejectCode": )";
    builder << '"' << writer.establishmentRejectCode() << '"';
