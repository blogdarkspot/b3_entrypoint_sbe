...
#ifndef _B3_ENTRYPOINT_FIXP_SBE_PERCENTAGEOPTIONAL_CXX_H_
#define _B3_ENTRYPOINT_FIXP_SBE_PERCENTAGEOPTIONAL_CXX_H_
...
public:
    ...
    PercentageOptional(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        PercentageOptional(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    PercentageOptional(
        char *buffer,
        const std::uint64_t bufferLength) :
        PercentageOptional(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }

    PercentageOptional &wrap(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    {
        return *this = PercentageOptional(buffer, offset, bufferLength, actingVersion);
    }
    ...
    PercentageOptional &mantissa(const std::int64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, PercentageOptional &writer)
{
    builder << '{';
    builder << R"("mantissa": )";
