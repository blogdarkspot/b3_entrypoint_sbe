...
#ifndef _B3_ENTRYPOINT_FIXP_SBE_CREDENTIALSENCODING_CXX_H_
#define _B3_ENTRYPOINT_FIXP_SBE_CREDENTIALSENCODING_CXX_H_
...
public:
    ...
    CredentialsEncoding(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        CredentialsEncoding(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    CredentialsEncoding(
        char *buffer,
        const std::uint64_t bufferLength) :
        CredentialsEncoding(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }

    CredentialsEncoding &wrap(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    {
        return *this = CredentialsEncoding(buffer, offset, bufferLength, actingVersion);
    }
    ...
    CredentialsEncoding &length(const std::uint8_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, CredentialsEncoding &writer)
{
    builder << '{';
    builder << R"("length": )";
