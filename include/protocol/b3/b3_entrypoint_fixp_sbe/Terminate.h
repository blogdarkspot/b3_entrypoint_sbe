...
#ifndef _B3_ENTRYPOINT_FIXP_SBE_TERMINATE_CXX_H_
#define _B3_ENTRYPOINT_FIXP_SBE_TERMINATE_CXX_H_
...
public:
    ...
    Terminate(char *buffer, const std::uint64_t bufferLength) :
        Terminate(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    Terminate(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        Terminate(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    Terminate &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        return *this = Terminate(buffer, offset, bufferLength, sbeBlockLength(), sbeSchemaVersion());
    }

    Terminate &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    Terminate &wrapForDecode(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = Terminate(buffer, offset, bufferLength, actingBlockLength, actingVersion);
    }
    ...
    {
        Terminate skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    {
        return MessageType::Value::Terminate;
    }
    ...
    {
        return static_cast<std::uint8_t>(MessageType::Value::Terminate);
    }
    ...
    Terminate &sessionID(const std::uint32_t value) SBE_NOEXCEPT
    {
    ...
    Terminate &sessionVerID(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    SBE_NODISCARD static const char *terminationCodeMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "required";
    ...
    static SBE_CONSTEXPR std::uint16_t terminationCodeId() SBE_NOEXCEPT
    {
        return 35533;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t terminationCodeSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool terminationCodeInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t terminationCodeEncodingOffset() SBE_NOEXCEPT
    {
        return 12;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t terminationCodeEncodingLength() SBE_NOEXCEPT
    {
        return 1;
    }

    SBE_NODISCARD std::uint8_t terminationCodeRaw() const SBE_NOEXCEPT
    {
        std::uint8_t val;
    ...
    SBE_NODISCARD TerminationCode::Value terminationCode() const
    {
        std::uint8_t val;
        std::memcpy(&val, m_buffer + m_offset + 12, sizeof(std::uint8_t));
        return TerminationCode::get((val));
    }

    Terminate &terminationCode(const TerminationCode::Value value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, Terminate &_writer)
{
    Terminate writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << ", ";
    builder << R"("terminationCode": )";
    builder << '"' << writer.terminationCode() << '"';
