...
#ifndef _B3_ENTRYPOINT_FIXP_SBE_NEWORDERSINGLE_CXX_H_
#define _B3_ENTRYPOINT_FIXP_SBE_NEWORDERSINGLE_CXX_H_
...
public:
    static const std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(127);
    static const std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(102);
    ...
    {
        return MessageType::Value::NewOrderSingle;
    }
    ...
    {
        return static_cast<std::uint8_t>(MessageType::Value::NewOrderSingle);
    }
...
public:
    ...
    NewOrderSingle &ordTagID(const std::uint8_t value) SBE_NOEXCEPT
    {
    ...
    NewOrderSingle &mmProtectionReset(const Boolean::Value value) SBE_NOEXCEPT
    {
        std::uint8_t val = (value);
    ...
    NewOrderSingle &account(const std::uint32_t value) SBE_NOEXCEPT
    {
    ...
    NewOrderSingle &senderLocation(const std::uint64_t index, const char value)
    {
        if (index >= 10)
        {
    ...
    NewOrderSingle &putSenderLocation(const char *const src) SBE_NOEXCEPT
    {
    ...
    #if __cplusplus >= 201703L
    NewOrderSingle &putSenderLocation(const std::string_view str)
    {
    ...
    #else
    NewOrderSingle &putSenderLocation(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 10)
        {
    ...
    NewOrderSingle &enteringTrader(const std::uint64_t index, const char value)
    {
        if (index >= 5)
        {
    ...
    NewOrderSingle &putEnteringTrader(const char *const src) SBE_NOEXCEPT
    {
    ...
    #if __cplusplus >= 201703L
    NewOrderSingle &putEnteringTrader(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 5)
        {
    ...
    #else
    NewOrderSingle &putEnteringTrader(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 5)
        {
    ...
    NewOrderSingle &selfTradePreventionInstruction(const SelfTradePreventionInstruction::Value value) SBE_NOEXCEPT
    {
        std::uint8_t val = (value);
    ...
    NewOrderSingle &securityID(const std::uint64_t value) SBE_NOEXCEPT
    {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    NewOrderSingle &side(const Side::Value value) SBE_NOEXCEPT
    {
        char val = (value);
    ...
    NewOrderSingle &ordType(const OrdType::Value value) SBE_NOEXCEPT
    {
        char val = (value);
    ...
    NewOrderSingle &timeInForce(const TimeInForce::Value value) SBE_NOEXCEPT
    {
        char val = (value);
    ...
    NewOrderSingle &routingInstruction(const RoutingInstruction::Value value) SBE_NOEXCEPT
    {
        std::uint8_t val = (value);
    ...
    NewOrderSingle &orderQty(const std::uint64_t value) SBE_NOEXCEPT
    {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
...
public:
    ...
    {
        m_stopPx.wrap(m_buffer, m_offset + 76, m_actingVersion, m_bufferLength);
        return m_stopPx;
    }
    ...
    NewOrderSingle &minQty(const std::uint64_t value) SBE_NOEXCEPT
    {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    NewOrderSingle &maxFloor(const std::uint64_t value) SBE_NOEXCEPT
    {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    SBE_NODISCARD char executingTrader(const std::uint64_t index) const
    {
        ...
        char val;
        std::memcpy(&val, m_buffer + m_offset + 100 + (index * 1), sizeof(char));
        return (val);
    }

    NewOrderSingle &executingTrader(const std::uint64_t index, const char value)
    {
        if (index >= 5)
        {
        ...
        char val = (value);
        std::memcpy(m_buffer + m_offset + 100 + (index * 1), &val, sizeof(char));
        return *this;
    }
    ...
    std::uint64_t getExecutingTrader(char *const dst, const std::uint64_t length) const
    {
        ...
        std::memcpy(dst, m_buffer + m_offset + 100, sizeof(char) * static_cast<std::size_t>(length));
        return length;
    }

    NewOrderSingle &putExecutingTrader(const char *const src) SBE_NOEXCEPT
    {
        std::memcpy(m_buffer + m_offset + 100, src, sizeof(char) * 5);
        return *this;
    }

    SBE_NODISCARD std::string getExecutingTraderAsString() const
    {
        const char *buffer = m_buffer + m_offset + 100;
        std::size_t length = 0;
    ...
    #if __cplusplus >= 201703L
    NewOrderSingle &putExecutingTrader(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 5)
        {
        ...
        std::memcpy(m_buffer + m_offset + 100, str.data(), srcLength);
        for (std::size_t start = srcLength; start < 5; ++start)
        {
            m_buffer[m_offset + 100 + start] = 0;
        }

        return *this;
    }
    #else
    NewOrderSingle &putExecutingTrader(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 5)
        {
        ...
        std::memcpy(m_buffer + m_offset + 100, str.c_str(), srcLength);
        for (std::size_t start = srcLength; start < 5; ++start)
        {
            m_buffer[m_offset + 100 + start] = 0;
        }
    ...
    {
        std::uint16_t val;
        std::memcpy(&val, m_buffer + m_offset + 105, sizeof(std::uint16_t));
        return SBE_LITTLE_ENDIAN_ENCODE_16(val);
    }

    NewOrderSingle &expireDate(const std::uint16_t value) SBE_NOEXCEPT
    {
        std::uint16_t val = SBE_LITTLE_ENDIAN_ENCODE_16(value);
        std::memcpy(m_buffer + m_offset + 105, &val, sizeof(std::uint16_t));
        return *this;
    }

    SBE_NODISCARD static const char *custodianInfoMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "optional";
    ...
    static SBE_CONSTEXPR std::uint16_t custodianInfoId() SBE_NOEXCEPT
    {
        return 35507;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t custodianInfoSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool custodianInfoInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t custodianInfoEncodingOffset() SBE_NOEXCEPT
    {
        return 107;
    }

private:
    CustodianInfo m_custodianInfo;

public:
    SBE_NODISCARD CustodianInfo &custodianInfo()
    {
        m_custodianInfo.wrap(m_buffer, m_offset + 107, m_actingVersion, m_bufferLength);
        return m_custodianInfo;
    }
...
public:
    ...
    {
        m_investorID.wrap(m_buffer, m_offset + 119, m_actingVersion, m_bufferLength);
        return m_investorID;
    }
    ...
    NewOrderSingle &putDeskID(const char *src, const std::uint8_t length)
    {
        std::uint64_t lengthOfLengthField = 1;
        std::uint64_t lengthPosition = sbePosition();
    ...
    NewOrderSingle &putDeskID(const std::string &str)
    {
        if (str.length() > 20)
        {
    ...
    #if __cplusplus >= 201703L
    NewOrderSingle &putDeskID(const std::string_view str)
    {
        if (str.length() > 20)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    NewOrderSingle &putMemo(const char *src, const std::uint8_t length)
    {
        std::uint64_t lengthOfLengthField = 1;
        std::uint64_t lengthPosition = sbePosition();
        std::uint8_t lengthFieldValue = (length);
    ...
    NewOrderSingle &putMemo(const std::string &str)
    {
        if (str.length() > 40)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    #if __cplusplus >= 201703L
    NewOrderSingle &putMemo(const std::string_view str)
    {
        if (str.length() > 40)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    std::basic_ostream<CharT, Traits> &builder, NewOrderSingle &_writer)
{
    ...
    builder << ", ";
    builder << R"("custodianInfo": )";
    builder << writer.custodianInfo();
