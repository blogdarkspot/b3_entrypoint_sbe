...
#ifndef _B3_ENTRYPOINT_FIXP_SBE_PERCENTAGE_CXX_H_
#define _B3_ENTRYPOINT_FIXP_SBE_PERCENTAGE_CXX_H_
...
public:
    ...
    Percentage(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        Percentage(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    Percentage(
        char *buffer,
        const std::uint64_t bufferLength) :
        Percentage(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }
    ...
    Percentage &wrap(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = Percentage(buffer, offset, bufferLength, actingVersion);
    }
    ...
    Percentage &mantissa(const std::int64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, Percentage &writer)
{
    builder << '{';
    builder << R"("mantissa": )";
