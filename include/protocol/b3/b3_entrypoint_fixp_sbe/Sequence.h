...
#ifndef _B3_ENTRYPOINT_FIXP_SBE_SEQUENCE_CXX_H_
#define _B3_ENTRYPOINT_FIXP_SBE_SEQUENCE_CXX_H_
...
public:
    ...
    Sequence(char *buffer, const std::uint64_t bufferLength) :
        Sequence(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    Sequence(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        Sequence(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    Sequence &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        return *this = Sequence(buffer, offset, bufferLength, sbeBlockLength(), sbeSchemaVersion());
    }

    Sequence &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    Sequence &wrapForDecode(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = Sequence(buffer, offset, bufferLength, actingBlockLength, actingVersion);
    }
    ...
    {
        Sequence skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    {
        return MessageType::Value::Sequence;
    }
    ...
    {
        return static_cast<std::uint8_t>(MessageType::Value::Sequence);
    }
    ...
    Sequence &nextSeqNo(const std::uint32_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, Sequence &_writer)
{
    Sequence writer(
        _writer.m_buffer,
        _writer.m_offset,
