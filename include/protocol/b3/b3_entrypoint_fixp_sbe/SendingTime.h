...
#ifndef _B3_ENTRYPOINT_FIXP_SBE_SENDINGTIME_CXX_H_
#define _B3_ENTRYPOINT_FIXP_SBE_SENDINGTIME_CXX_H_
...
public:
    ...
    SendingTime(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        SendingTime(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    SendingTime(
        char *buffer,
        const std::uint64_t bufferLength) :
        SendingTime(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }
    ...
    SendingTime &wrap(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = SendingTime(buffer, offset, bufferLength, actingVersion);
    }
    ...
    SendingTime &time(const std::uint64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, SendingTime &writer)
{
    builder << '{';
    builder << R"("time": )";
    builder << +writer.time();
