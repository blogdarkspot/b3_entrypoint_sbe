...
#ifndef _B3_ENTRYPOINT_FIXP_SBE_INBOUNDBUSINESSHEADER_CXX_H_
#define _B3_ENTRYPOINT_FIXP_SBE_INBOUNDBUSINESSHEADER_CXX_H_
...
public:
    ...
    InboundBusinessHeader() = default;

    InboundBusinessHeader(
        char *buffer,
    ...
    InboundBusinessHeader(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        InboundBusinessHeader(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    InboundBusinessHeader(
        char *buffer,
        const std::uint64_t bufferLength) :
        InboundBusinessHeader(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }

    InboundBusinessHeader &wrap(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    {
        return *this = InboundBusinessHeader(buffer, offset, bufferLength, actingVersion);
    }
    ...
    InboundBusinessHeader &sessionID(const std::uint32_t value) SBE_NOEXCEPT
    {
    ...
    InboundBusinessHeader &msgSeqNum(const std::uint32_t value) SBE_NOEXCEPT
    {
        std::uint32_t val = SBE_LITTLE_ENDIAN_ENCODE_32(value);
...
public:
    ...
    InboundBusinessHeader &marketSegmentID(const std::uint8_t value) SBE_NOEXCEPT
    {
    ...
    InboundBusinessHeader &padding(const char value) SBE_NOEXCEPT
    {
        char val = (value);
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, InboundBusinessHeader &writer)
{
    builder << '{';
    builder << R"("sessionID": )";
