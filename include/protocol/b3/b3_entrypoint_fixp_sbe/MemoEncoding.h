...
#ifndef _B3_ENTRYPOINT_FIXP_SBE_MEMOENCODING_CXX_H_
#define _B3_ENTRYPOINT_FIXP_SBE_MEMOENCODING_CXX_H_
...
public:
    ...
    MemoEncoding(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        MemoEncoding(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    MemoEncoding(
        char *buffer,
        const std::uint64_t bufferLength) :
        MemoEncoding(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }
    ...
    MemoEncoding &wrap(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = MemoEncoding(buffer, offset, bufferLength, actingVersion);
    }
    ...
    MemoEncoding &length(const std::uint8_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, MemoEncoding &writer)
{
    builder << '{';
    builder << R"("length": )";
