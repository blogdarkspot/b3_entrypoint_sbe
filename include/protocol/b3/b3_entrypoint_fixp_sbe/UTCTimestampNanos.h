...
#ifndef _B3_ENTRYPOINT_FIXP_SBE_UTCTIMESTAMPNANOS_CXX_H_
#define _B3_ENTRYPOINT_FIXP_SBE_UTCTIMESTAMPNANOS_CXX_H_
...
public:
    ...
    UTCTimestampNanos(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        UTCTimestampNanos(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    UTCTimestampNanos(
        char *buffer,
        const std::uint64_t bufferLength) :
        UTCTimestampNanos(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }

    UTCTimestampNanos &wrap(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    {
        return *this = UTCTimestampNanos(buffer, offset, bufferLength, actingVersion);
    }
    ...
    UTCTimestampNanos &time(const std::uint64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, UTCTimestampNanos &writer)
{
    builder << '{';
    builder << R"("time": )";
    builder << +writer.time();
