...
#ifndef _B3_ENTRYPOINT_FIXP_SBE_RETRANSMISSION_CXX_H_
#define _B3_ENTRYPOINT_FIXP_SBE_RETRANSMISSION_CXX_H_
...
public:
    ...
    Retransmission(char *buffer, const std::uint64_t bufferLength) :
        Retransmission(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    Retransmission(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        Retransmission(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    Retransmission &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        return *this = Retransmission(buffer, offset, bufferLength, sbeBlockLength(), sbeSchemaVersion());
    }

    Retransmission &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    Retransmission &wrapForDecode(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = Retransmission(buffer, offset, bufferLength, actingBlockLength, actingVersion);
    }

    Retransmission &sbeRewind()
    {
    ...
    {
        Retransmission skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    {
        return MessageType::Value::Retransmission;
    }
    ...
    {
        return static_cast<std::uint8_t>(MessageType::Value::Retransmission);
    }
    ...
    Retransmission &sessionID(const std::uint32_t value) SBE_NOEXCEPT
    {
...
public:
    ...
    Retransmission &nextSeqNo(const std::uint32_t value) SBE_NOEXCEPT
    {
        std::uint32_t val = SBE_LITTLE_ENDIAN_ENCODE_32(value);
    ...
    Retransmission &count(const std::uint32_t value) SBE_NOEXCEPT
    {
        std::uint32_t val = SBE_LITTLE_ENDIAN_ENCODE_32(value);
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, Retransmission &_writer)
{
    Retransmission writer(
        _writer.m_buffer,
        _writer.m_offset,
