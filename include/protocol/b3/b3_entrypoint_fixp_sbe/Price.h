...
#ifndef _B3_ENTRYPOINT_FIXP_SBE_PRICE_CXX_H_
#define _B3_ENTRYPOINT_FIXP_SBE_PRICE_CXX_H_
...
public:
    ...
    Price(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        Price(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    Price(
        char *buffer,
        const std::uint64_t bufferLength) :
        Price(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }
    ...
    Price &wrap(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = Price(buffer, offset, bufferLength, actingVersion);
    }
    ...
    Price &mantissa(const std::int64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, Price &writer)
{
    builder << '{';
    builder << R"("mantissa": )";
