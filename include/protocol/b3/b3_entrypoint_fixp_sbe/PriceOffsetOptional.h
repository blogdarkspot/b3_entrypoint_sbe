...
#ifndef _B3_ENTRYPOINT_FIXP_SBE_PRICEOFFSETOPTIONAL_CXX_H_
#define _B3_ENTRYPOINT_FIXP_SBE_PRICEOFFSETOPTIONAL_CXX_H_
...
public:
    ...
    PriceOffsetOptional(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        PriceOffsetOptional(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    PriceOffsetOptional(
        char *buffer,
        const std::uint64_t bufferLength) :
        PriceOffsetOptional(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }

    PriceOffsetOptional &wrap(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    {
        return *this = PriceOffsetOptional(buffer, offset, bufferLength, actingVersion);
    }
    ...
    PriceOffsetOptional &mantissa(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    {
        return static_cast<std::int8_t>(-4);
    }

template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, PriceOffsetOptional &writer)
{
    builder << '{';
    builder << R"("mantissa": )";
