...
#ifndef _B3_ENTRYPOINT_FIXP_SBE_DELTAINMILLIS_CXX_H_
#define _B3_ENTRYPOINT_FIXP_SBE_DELTAINMILLIS_CXX_H_
...
public:
    ...
    DeltaInMillis(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        DeltaInMillis(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    DeltaInMillis(
        char *buffer,
        const std::uint64_t bufferLength) :
        DeltaInMillis(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }
    ...
    DeltaInMillis &wrap(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = DeltaInMillis(buffer, offset, bufferLength, actingVersion);
    }
    ...
    DeltaInMillis &time(const std::uint64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, DeltaInMillis &writer)
{
    builder << '{';
    builder << R"("time": )";
    builder << +writer.time();
