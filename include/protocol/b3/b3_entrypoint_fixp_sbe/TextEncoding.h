...
#ifndef _B3_ENTRYPOINT_FIXP_SBE_TEXTENCODING_CXX_H_
#define _B3_ENTRYPOINT_FIXP_SBE_TEXTENCODING_CXX_H_
...
public:
    ...
    TextEncoding(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        TextEncoding(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    TextEncoding(
        char *buffer,
        const std::uint64_t bufferLength) :
        TextEncoding(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }
    ...
    TextEncoding &wrap(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = TextEncoding(buffer, offset, bufferLength, actingVersion);
    }
    ...
    {
        return static_cast<std::uint8_t>(250);
    }
    ...
    TextEncoding &length(const std::uint8_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, TextEncoding &writer)
{
    builder << '{';
    builder << R"("length": )";
