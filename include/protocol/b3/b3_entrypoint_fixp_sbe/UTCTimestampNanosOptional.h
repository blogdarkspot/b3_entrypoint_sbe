...
#ifndef _B3_ENTRYPOINT_FIXP_SBE_UTCTIMESTAMPNANOSOPTIONAL_CXX_H_
#define _B3_ENTRYPOINT_FIXP_SBE_UTCTIMESTAMPNANOSOPTIONAL_CXX_H_
...
public:
    ...
    UTCTimestampNanosOptional() = default;
    ...
    UTCTimestampNanosOptional(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        UTCTimestampNanosOptional(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    UTCTimestampNanosOptional(
        char *buffer,
        const std::uint64_t bufferLength) :
        UTCTimestampNanosOptional(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }

    UTCTimestampNanosOptional &wrap(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    {
        return *this = UTCTimestampNanosOptional(buffer, offset, bufferLength, actingVersion);
    }
    ...
    UTCTimestampNanosOptional &time(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    SBE_NODISCARD static const char *unitMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
    ...
    static SBE_CONSTEXPR std::uint16_t unitId() SBE_NOEXCEPT
    {
        return -1;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t unitSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool unitInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t unitEncodingOffset() SBE_NOEXCEPT
    {
        return 8;
    }
    ...
    SBE_NODISCARD static SBE_CONSTEXPR std::uint8_t unit() SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, UTCTimestampNanosOptional &writer)
{
    builder << '{';
    builder << R"("time": )";
    builder << +writer.time();
