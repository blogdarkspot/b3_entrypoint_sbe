...
#ifndef _B3_ENTRYPOINT_FIXP_SBE_ESTABLISH_CXX_H_
#define _B3_ENTRYPOINT_FIXP_SBE_ESTABLISH_CXX_H_
...
public:
    ...
    {
        return *this = Establish(buffer, offset, bufferLength, sbeBlockLength(), sbeSchemaVersion());
    }
    ...
    Establish &wrapForDecode(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = Establish(buffer, offset, bufferLength, actingBlockLength, actingVersion);
    }
    ...
    {
        Establish skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    {
        return MessageType::Value::Establish;
    }
    ...
    {
        return static_cast<std::uint8_t>(MessageType::Value::Establish);
    }
    ...
    Establish &sessionID(const std::uint32_t value) SBE_NOEXCEPT
    {
    ...
    Establish &sessionVerID(const std::uint64_t value) SBE_NOEXCEPT
    {
...
public:
    ...
    Establish &nextSeqNo(const std::uint32_t value) SBE_NOEXCEPT
    {
        std::uint32_t val = SBE_LITTLE_ENDIAN_ENCODE_32(value);
    ...
    SBE_NODISCARD static const char *cancelOnDisconnectTypeMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "required";
    ...
    static SBE_CONSTEXPR std::uint16_t cancelOnDisconnectTypeId() SBE_NOEXCEPT
    {
        return 35002;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t cancelOnDisconnectTypeSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool cancelOnDisconnectTypeInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t cancelOnDisconnectTypeEncodingOffset() SBE_NOEXCEPT
    {
        return 32;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t cancelOnDisconnectTypeEncodingLength() SBE_NOEXCEPT
    {
        return 1;
    }

    SBE_NODISCARD std::uint8_t cancelOnDisconnectTypeRaw() const SBE_NOEXCEPT
    {
        std::uint8_t val;
    ...
    SBE_NODISCARD CancelOnDisconnectType::Value cancelOnDisconnectType() const
    {
        std::uint8_t val;
        std::memcpy(&val, m_buffer + m_offset + 32, sizeof(std::uint8_t));
        return CancelOnDisconnectType::get((val));
    }

    Establish &cancelOnDisconnectType(const CancelOnDisconnectType::Value value) SBE_NOEXCEPT
    {
    ...
    SBE_NODISCARD static const char *codTimeoutWindowMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "required";
    ...
    static SBE_CONSTEXPR std::uint16_t codTimeoutWindowId() SBE_NOEXCEPT
    {
        return 35003;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t codTimeoutWindowSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool codTimeoutWindowInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t codTimeoutWindowEncodingOffset() SBE_NOEXCEPT
    {
        return 34;
    }

private:
    DeltaInMillis m_codTimeoutWindow;

public:
    SBE_NODISCARD DeltaInMillis &codTimeoutWindow()
    {
        m_codTimeoutWindow.wrap(m_buffer, m_offset + 34, m_actingVersion, m_bufferLength);
        return m_codTimeoutWindow;
    }
    ...
    Establish &putCredentials(const char *src, const std::uint8_t length)
    {
        std::uint64_t lengthOfLengthField = 1;
        std::uint64_t lengthPosition = sbePosition();
    ...
    Establish &putCredentials(const std::string &str)
    {
        if (str.length() > 128)
        {
    ...
    #if __cplusplus >= 201703L
    Establish &putCredentials(const std::string_view str)
    {
        if (str.length() > 128)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, Establish &_writer)
{
    Establish writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << ", ";
    builder << R"("cancelOnDisconnectType": )";
    builder << '"' << writer.cancelOnDisconnectType() << '"';

    builder << ", ";
    builder << R"("codTimeoutWindow": )";
    builder << writer.codTimeoutWindow();
