...
#ifndef _B3_ENTRYPOINT_FIXP_SBE_PRICEOPTIONAL_CXX_H_
#define _B3_ENTRYPOINT_FIXP_SBE_PRICEOPTIONAL_CXX_H_
...
public:
    ...
    PriceOptional(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        PriceOptional(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    PriceOptional(
        char *buffer,
        const std::uint64_t bufferLength) :
        PriceOptional(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }
    ...
    PriceOptional &wrap(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = PriceOptional(buffer, offset, bufferLength, actingVersion);
    }
    ...
    PriceOptional &mantissa(const std::int64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, PriceOptional &writer)
{
    builder << '{';
    builder << R"("mantissa": )";
