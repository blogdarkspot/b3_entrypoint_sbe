...
#ifndef _B3_ENTRYPOINT_FIXP_SBE_RETRANSMITREJECT_CXX_H_
#define _B3_ENTRYPOINT_FIXP_SBE_RETRANSMITREJECT_CXX_H_
...
public:
    ...
    RetransmitReject(char *buffer, const std::uint64_t bufferLength) :
        RetransmitReject(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    RetransmitReject(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        RetransmitReject(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    RetransmitReject &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        return *this = RetransmitReject(buffer, offset, bufferLength, sbeBlockLength(), sbeSchemaVersion());
    }

    RetransmitReject &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    RetransmitReject &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    {
        return *this = RetransmitReject(buffer, offset, bufferLength, actingBlockLength, actingVersion);
    }

    RetransmitReject &sbeRewind()
    {
    ...
    {
        RetransmitReject skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    {
        return MessageType::Value::RetransmitReject;
    }
    ...
    {
        return static_cast<std::uint8_t>(MessageType::Value::RetransmitReject);
    }
    ...
    RetransmitReject &sessionID(const std::uint32_t value) SBE_NOEXCEPT
    {
...
public:
    ...
    {
        m_requestTimestamp.wrap(m_buffer, m_offset + 4, m_actingVersion, m_bufferLength);
        return m_requestTimestamp;
    }

    SBE_NODISCARD static const char *retransmitRejectCodeMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "required";
    ...
    static SBE_CONSTEXPR std::uint16_t retransmitRejectCodeId() SBE_NOEXCEPT
    {
        return 35534;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t retransmitRejectCodeSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool retransmitRejectCodeInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t retransmitRejectCodeEncodingOffset() SBE_NOEXCEPT
    {
        return 12;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t retransmitRejectCodeEncodingLength() SBE_NOEXCEPT
    {
        return 1;
    }

    SBE_NODISCARD std::uint8_t retransmitRejectCodeRaw() const SBE_NOEXCEPT
    {
        std::uint8_t val;
    ...
    SBE_NODISCARD RetransmitRejectCode::Value retransmitRejectCode() const
    {
        std::uint8_t val;
        std::memcpy(&val, m_buffer + m_offset + 12, sizeof(std::uint8_t));
        return RetransmitRejectCode::get((val));
    }

    RetransmitReject &retransmitRejectCode(const RetransmitRejectCode::Value value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, RetransmitReject &_writer)
{
    RetransmitReject writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "RetransmitReject", )";
    ...
    builder << ", ";
    builder << R"("retransmitRejectCode": )";
    builder << '"' << writer.retransmitRejectCode() << '"';
