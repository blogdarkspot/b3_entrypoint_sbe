...
#ifndef _B3_ENTRYPOINT_FIXP_SBE_SIMPLENEWORDER_CXX_H_
#define _B3_ENTRYPOINT_FIXP_SBE_SIMPLENEWORDER_CXX_H_
...
public:
    ...
    static const std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(100);
    ...
    SimpleNewOrder(char *buffer, const std::uint64_t bufferLength) :
        SimpleNewOrder(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    SimpleNewOrder(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        SimpleNewOrder(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    SimpleNewOrder &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        return *this = SimpleNewOrder(buffer, offset, bufferLength, sbeBlockLength(), sbeSchemaVersion());
    }

    SimpleNewOrder &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    SimpleNewOrder &wrapForDecode(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = SimpleNewOrder(buffer, offset, bufferLength, actingBlockLength, actingVersion);
    }

    SimpleNewOrder &sbeRewind()
    {
    ...
    {
        SimpleNewOrder skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    {
        return MessageType::Value::SimpleNewOrder;
    }
    ...
    {
        return static_cast<std::uint8_t>(MessageType::Value::SimpleNewOrder);
    }
...
public:
    ...
    SimpleNewOrder &ordTagID(const std::uint8_t value) SBE_NOEXCEPT
    {
    ...
    SimpleNewOrder &mmProtectionReset(const Boolean::Value value) SBE_NOEXCEPT
    {
        std::uint8_t val = (value);
    ...
    SimpleNewOrder &clOrdID(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    SimpleNewOrder &account(const std::uint32_t value) SBE_NOEXCEPT
    {
    ...
    SimpleNewOrder &senderLocation(const std::uint64_t index, const char value)
    {
        if (index >= 10)
        {
    ...
    SimpleNewOrder &putSenderLocation(const char *const src) SBE_NOEXCEPT
    {
    ...
    #if __cplusplus >= 201703L
    SimpleNewOrder &putSenderLocation(const std::string_view str)
    {
    ...
    #else
    SimpleNewOrder &putSenderLocation(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 10)
        {
    ...
    SimpleNewOrder &enteringTrader(const std::uint64_t index, const char value)
    {
        if (index >= 5)
        {
    ...
    SimpleNewOrder &putEnteringTrader(const char *const src) SBE_NOEXCEPT
    {
    ...
    #if __cplusplus >= 201703L
    SimpleNewOrder &putEnteringTrader(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 5)
        {
    ...
    #else
    SimpleNewOrder &putEnteringTrader(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 5)
        {
    ...
    SimpleNewOrder &selfTradePreventionInstruction(const SelfTradePreventionInstruction::Value value) SBE_NOEXCEPT
    {
        std::uint8_t val = (value);
    ...
    SimpleNewOrder &securityID(const std::uint64_t value) SBE_NOEXCEPT
    {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    SimpleNewOrder &side(const Side::Value value) SBE_NOEXCEPT
    {
        char val = (value);
    ...
    SimpleNewOrder &ordType(const SimpleOrdType::Value value) SBE_NOEXCEPT
    {
        char val = (value);
    ...
    SimpleNewOrder &timeInForce(const SimpleTimeInForce::Value value) SBE_NOEXCEPT
    {
        char val = (value);
    ...
    SimpleNewOrder &routingInstruction(const RoutingInstruction::Value value) SBE_NOEXCEPT
    {
        std::uint8_t val = (value);
    ...
    SimpleNewOrder &orderQty(const std::uint64_t value) SBE_NOEXCEPT
    {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
...
public:
    ...
    {
        m_investorID.wrap(m_buffer, m_offset + 76, m_actingVersion, m_bufferLength);
        return m_investorID;
    }
    ...
    SimpleNewOrder &putMemo(const char *src, const std::uint8_t length)
    {
        std::uint64_t lengthOfLengthField = 1;
        std::uint64_t lengthPosition = sbePosition();
    ...
    SimpleNewOrder &putMemo(const std::string &str)
    {
        if (str.length() > 40)
        {
    ...
    #if __cplusplus >= 201703L
    SimpleNewOrder &putMemo(const std::string_view str)
    {
        if (str.length() > 40)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, SimpleNewOrder &_writer)
{
    SimpleNewOrder writer(
        _writer.m_buffer,
        _writer.m_offset,
