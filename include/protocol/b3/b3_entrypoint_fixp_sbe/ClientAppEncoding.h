...
#ifndef _B3_ENTRYPOINT_FIXP_SBE_CLIENTAPPENCODING_CXX_H_
#define _B3_ENTRYPOINT_FIXP_SBE_CLIENTAPPENCODING_CXX_H_
...
public:
    ...
    ClientAppEncoding(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        ClientAppEncoding(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    ClientAppEncoding(
        char *buffer,
        const std::uint64_t bufferLength) :
        ClientAppEncoding(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }

    ClientAppEncoding &wrap(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    {
        return *this = ClientAppEncoding(buffer, offset, bufferLength, actingVersion);
    }
    ...
    static SBE_CONSTEXPR std::uint8_t lengthNullValue() SBE_NOEXCEPT
    {
    ...
    ClientAppEncoding &length(const std::uint8_t value) SBE_NOEXCEPT
    {
    ...
    SBE_NODISCARD static const char *varDataMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "required";
    ...
    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t varDataSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }
    ...
    static SBE_CONSTEXPR char varDataNullValue() SBE_NOEXCEPT
    {
        return static_cast<char>(0);
    }

    static SBE_CONSTEXPR char varDataMinValue() SBE_NOEXCEPT
    {
        return static_cast<char>(32);
    }

    static SBE_CONSTEXPR char varDataMaxValue() SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, ClientAppEncoding &writer)
{
    builder << '{';
    builder << R"("length": )";
